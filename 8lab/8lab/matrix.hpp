...
#define MY_DEBUG 
	...
	public:
		...
		{
#ifdef MY_DEBUG
			std::cout << "Опреатор сложения" << std::endl;
#endif
			Matrix<T, N, M> tmp;
			for (int i = 0; i < m_n; i++)
				for (int j = 0; j < m_m; j++)
		...
		{
#ifdef MY_DEBUG
			std::cout << "Определитель" << std::endl; // Определитель
#endif
		...
		Matrix <T, N, M> Rev() // обратная матрица
		{
#ifdef MY_DEBUG
		...
		Matrix <T, N, M> Trans() //транспонирование матрицы
		{
#ifdef MY_DEBUG
			std::cout << "Транспонирование матрицы" << std::endl;
#endif
			Matrix<T, N, M> tmp;
			for (int i = 0; i < m_n; i++)
				for (int j = 0; j < m_m; j++)
		...
		{
#ifdef MY_DEBUG
			std::cout << "Деструктор" << std::endl; // деструктор матрицы
#endif
		}
